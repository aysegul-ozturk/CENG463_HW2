...
df = pd.read_csv("./trainingset-ideology-power/orientation/orientation-es-train.tsv", sep="\t")
...
def preprocess_data(examples):
    return tokenizer(
        examples["text_en"], padding="max_length", truncation=True
    )
...
def remove_unused_columns(dataset):
    columns_to_remove = [col for col in ["text_en", "__index_level_0__"] if col in dataset.column_names]
...
def classify_text_gpt3(speech):
    ...
    Based on the following speech excerpt, determine whether the political ideology of the speaker's party is left-leaning (0) 
    or right-leaning (1). 
